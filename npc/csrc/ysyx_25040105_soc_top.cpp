...
uint32_t pmem_read(uint32_t addr) {
    // 确保地址对齐并检查边界
    assert((addr & 0x3) == 0); // 地址必须是4字节对齐
    uint32_t index = (addr - 0x80000000) >> 2;  // 假设PC从0x80000000开始
    assert(index < PMEM_SIZE);
    return pmem[index];
}
...
int main(int argc, char** argv) {
    ...
    reset(5);
    top->inst = pmem_read(0x80000000); // 初始提供inst
    for (int i = 0; i < 20; ++i) {
        single_cycle(*top);
        top->inst = pmem_read(top->pc); // 准备下一条指令
        top->eval();
    }
